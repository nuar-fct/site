...
def criar_atividade():
    ...
    descricao = input("Descrição (aparece no pop-up): ").strip()

    # Foto principal (card)
    print("\n🖼 Foto principal do card:")
    print("Cole o link completo do Google Drive")
    foto_card_url_raw = input("URL da foto do card: ").strip()
    foto_card_thumb = drive_url_to_thumbnail(foto_card_url_raw)
    ...
    # Conteúdo estendido (opcional)
    print("\n📄 Conteúdo Estendido (opcional):")
    usar_ext = input("Queres adicionar conteúdo estendido? (s/n): ").strip().lower()
    ...
    atividade = {
        ...
        "cardImageUrl": foto_card_thumb,
        "drivePhotos": fotos,
    }

    if extended:
        atividade["extendedContent"] = extended
    
    return atividade


def gerar_html_card(atividade):
    """Gera o HTML do card da atividade (novo layout com imagem)."""
    thumb_url = atividade.get("cardImageUrl", "")

    return f'''
    <div class="activity-card" data-activity-id="{atividade['id']}">
        <div class="activity-card-image">
            <img src="{thumb_url}" alt="Imagem do evento {atividade['title']}">
        </div>
        ...
        <div class="activity-content">
            ...
            <button class="register-btn" onclick="openModal('{atividade['id']}')">Ver mais</button>
        </div>
    </div>
'''.strip("\n")
...
def gerar_js_data(atividade):
    ...
    # extendedContent opcional
    if "extendedContent" in atividade and atividade["extendedContent"]:
        topicos_js = ",\n                        ".join(
...
def drive_url_to_thumbnail(url):
    """
    Converte um link do Google Drive (file/d/ID/..., open?id=ID, uc?id=ID ou só o ID)
    para https://drive.google.com/thumbnail?id=ID&sz=w1000
    """
    if not url:
        return ""
    ...
    if "http" not in url:
        ...
        if m:
            ...
            if m:
                ...
                # fallback: sequência longa de [\w-]
                m = re.search(r'[-\w]{20,}', url)
                file_id = m.group(0) if m else url
...
def encontrar_id_por_titulo_data_html(conteudo, titulo, data):
    ...
    """
    pattern = re.compile(
        r'<div\s+class="activity-card"[^>]*data-activity-id="([^"]+)"[^>]*>'   # abre o card e captura id
        r'(?:(?!<div\s+class="activity-card").)*?'                             # só dentro deste card
        r'<h3\s+class="activity-title">\s*' + re.escape(titulo) + r'\s*</h3>'  # título certo
        r'(?:(?!<div\s+class="activity-card").)*?'                             # ainda dentro do mesmo card
        r'<span\s+class="detail-icon">📅</span>\s*'
        r'<span>\s*' + re.escape(data) + r'\s*</span>',
...
def menu_principal():
    ...
    while True:
        ...
        if opcao == "1":
            ...
            ext = atividade.get("extendedContent")
            num_topicos = len(ext.get("topics", [])) if ext else 0
            print(f"Tópicos: {num_topicos}")
